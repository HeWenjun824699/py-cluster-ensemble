...
from .methods.ecpcs_mc_core import ecpcs_mc_core
...
def ecpcs_mc(
    ...
    对应 MATLAB 脚本 run_ECPCSMC_t20_TSMC_2021.m 的主逻辑。
    ...
    theta : float, default=20
        ECPCS 算法中的参数 t (对应 MATLAB 中的 t = 20)。
        通常用于控制相似度传播或筛选的阈值。
    ...
    for iRepeat in range(nRepeat):
        ...
        try:
            # 调用核心算法
            # MATLAB: label = ECPCS_MC(BPi, t, nCluster);
            # 假设 Python 版 core 函数直接返回 label 数组
            label_pred = ecpcs_mc_core(BPi, theta, nCluster)

            # 确保输出是展平的 numpy array
            label_pred = np.array(label_pred).flatten()

        except Exception as e:
            print(f"ECPCS-MC failed on repeat {iRepeat}: {e}")
            # 发生错误时返回全零标签
