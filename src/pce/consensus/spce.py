...
def spce(
    ...
    对应 MATLAB 脚本 run_SPCE_TNNLS_2021.m 的主逻辑。

    该算法通常包含以下步骤:
    1. 将基聚类转换为共协矩阵张量 (Ai)
    2. 自步学习优化求解一致性矩阵 S (Optimize)
    3. 基于 S 构建图并计算连通分量 (ConnComp) 得到最终聚类
    ...
    gamma : float, default=0.5
        SPCE 算法中的自步学习参数 gamma (对应 MATLAB 中的 gamma = 0.5)
    nBase : int, default=20
        每次重复实验使用的基聚类器数量
    nRepeat : int, default=10
        实验重复次数
    seed : int, default=2026
        随机种子 (对应 MATLAB 脚本中的 seed = 2026)
    ...
    for iRepeat in range(nRepeat):
        ...
        try:
            # 调用核心算法
            # MATLAB 逻辑:
            # 1. 构建 Ai (Tensor construction)
            # 2. S = Optimize(Ai, nCluster, gamma)
            # 3. label = conncomp(graph(S))

            # 假设 Python 版 core 函数封装了上述张量构建、优化及连通分量提取步骤
